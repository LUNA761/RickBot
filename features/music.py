...
from helpers.spotify import Spotify, SpotifyGenerator
...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def create_source(cls, ctx: commands.Context, search: str, *, loop: asyncio.BaseEventLoop = None):
        ...
        # get rid of exe for rasp pi
        return cls(ctx, discord.FFmpegPCMAudio(info['url'], **cls.FFMPEG_OPTIONS, executable=r"C:/ffmpeg/bin/ffmpeg.exe"), data=info)
...
class Song:
    ...
    def create_embed(self):
        embed = (discord.Embed(title='Now playing',
                               description='```css\n{0.source.title}\n```'.format(self),
                               color=int(config.get("EMBEDS", "color"), 16))
...
class Music(commands.Cog):
    ...
    @commands.command(name='join', aliases=['hello', 'j', 'summon'])
    ...
    async def _skip(self, ctx: commands.Context):
        ...
        if voter == ctx.voice_state.current.requester:
            ...
            if total_votes >= skips_needed:
                ...
                await ctx.send(f"Skip vote added, currently at **{total_votes}/{skips_needed}**")
    ...
    async def _play(self, ctx: commands.Context, *, search: str = None):
        ...
        """
        try:
            if ctx.voice_client.is_paused() is True and not search:
                try:
        ...
        if "open.spotify.com" in search:
            if "/playlist/" in search or "/album/" in search:
                ...
                await ctx.send(f'Adding songs, this may take some time.')

                async with ctx.typing():
                    songs_queued = 0
                    for url in SpotifyGenerator.get_youtube_urls(_url):
                        try:
                ...
                await ctx.send(f'Enqueued {songs_queued} songs')

                return

            async with ctx.typing():
                _url = re.search("(?P<url>https?://[^\s]+)", search).group("url")
                urls = Spotify.get_youtube_urls(_url)
                ...
                if len(urls) == 0:
                    ...
                    try:
                        source = await YTDLSource.create_source(ctx, urls[0], loop=self.client.loop)
                    except YTDLError as e:
                        await ctx.send('An error occurred while processing this request: {}'.format(str(e)))
                    else:
                        song = Song(source)
